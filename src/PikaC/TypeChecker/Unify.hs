...
getRepr sigVars v eqs = do
  ...
  case lookupVarUnifyEq unifyEqs v of
    Nothing -> undefined
    Just (UnifyEq _ (Just (AdtName adt))) -> pure $ LayoutId adt
...
data UnifyEq = UnifyEq [TypeName] (Maybe AdtName)
  deriving (Show)
...
-- -- TODO: Make sure this is actually a semigroup
-- instance Semigroup UnifyEq where
--   x <> y =
--     case combine x y of
--       Just r -> r
--       Nothing -> y
...
combine (UnifyEq xs adt) (UnifyEq ys adt') = Just $ UnifyEq (xs ++ ys) (adt <|> adt')

singleUnifyEq :: [(String, AdtName)] -> Equal -> Maybe UnifyEq
singleUnifyEq layoutAdts (v :=: LayoutId n) =
  let Just adt = lookup n layoutAdts
  in
  Just $ UnifyEq [v] (Just adt)
singleUnifyEq layoutAdts (v :=: TyVar v') = Just $ UnifyEq [v, v'] Nothing
singleUnifyEq _ _ = Nothing


-- type Class = [(TypeName, TypeName)]
--
-- equivClasses :: [Equal] -> [Class]
-- equivClasses = undefined --foldr insertEqIntoClass []
--
-- insertEqIntoClass :: Equal -> Class -> Class
-- insertEqIntoClass (x :=: TyVar y) = insertIntoClass x y
-- insertEqIntoClass _               = id
--
-- insertIntoClass :: TypeName -> TypeName -> Class -> Class
-- insertIntoClass x y [] = [(x, y)]
-- insertIntoClass x y zs
--   | Just x' <- lookup x zs = (x, y) : zs
--   | otherwise              = zs
--
-- mergeClasses :: [Class] -> [Class]
-- mergeClasses [] = []
-- mergeClasses (x:xs) = mergeInto x xs
--
-- mergeInto :: Class -> [Class] -> [Class]
-- mergeInto x [] = [x]
-- mergeInto x (y:ys)
--   | any (`elem` map fst y) (map fst x) = mergeInto (union x y) ys
--   | otherwise                          = y : mergeInto x ys


-- -- | Make sure all the ADTs match
-- checkConsistency :: [Equal] -> Check s ()
-- checkConsistency = fmap checkAdts . go
--   where
--     go :: [Equal] -> Check s [(TypeName, Type)]
--     go [] = pure []
--     go ((v :=: _) : rest) = undefined
--     checkAdts = undefined
