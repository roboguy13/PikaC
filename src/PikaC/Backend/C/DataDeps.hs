...
instance Semigroup DataDeps where
  x <> y =
    DataDeps
...
instance Monoid DataDeps where
  mempty = DataDeps [] []

dataDepsLe :: DataDeps -> DataDeps -> Bool
dataDepsLe x y =
  all (`notElem` dataDepsWrites y) (dataDepsReads x)

topologicalSortByDataDeps :: (Ord a) => (a -> DataDeps) -> [a] -> [a]
topologicalSortByDataDeps f =
  topologicalSortBy (dataDepsLe `on` f)

type FnDataDeps a = [(String, [Loc] -> DataDeps)]
